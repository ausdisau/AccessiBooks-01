...
export async function trainCFModel(dataset: InteractionDataset): Promise<CFArtifacts | null> {
  ...
  try {
    ...
    console.log("[recommendation/cfModel] @tensorflow/tfjs-node not available, skipping CF training.");
    return null;
  }
  ...
  const userEmbedding = tf.layers
    ...
    .apply(userInput) as import("@tensorflow/tfjs-node").SymbolicTensor;
  const bookEmbedding = tf.layers
    .embedding({ inputDim: numBooks, outputDim: dim, embeddingsInitializer: "randomNormal", name: "bookEmb" })
    .apply(bookInput) as import("@tensorflow/tfjs-node").SymbolicTensor;

  const userVec = tf.layers.flatten().apply(userEmbedding) as import("@tensorflow/tfjs-node").SymbolicTensor;
  const bookVec = tf.layers.flatten().apply(bookEmbedding) as import("@tensorflow/tfjs-node").SymbolicTensor;
  const dotLayer = tf.layers.dot({ axes: -1 }).apply([userVec, bookVec]) as import("@tensorflow/tfjs-node").SymbolicTensor;
  const out = tf.layers.activation({ activation: "sigmoid" }).apply(dotLayer) as import("@tensorflow/tfjs-node").SymbolicTensor;
  ...
  if (history.history.loss && history.history.loss.length > 0) {
    ...
    lastLoss = typeof v === "number" ? v : (v as import("@tensorflow/tfjs-node").Scalar).dataSync()[0];
  }
