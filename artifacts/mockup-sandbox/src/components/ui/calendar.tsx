...
function Calendar({
  ...
  // Typed strictly against react-day-picker's current ClassNames keys (UI /
  // DayFlag / SelectionState / Animation enums). This intentionally excludes the
  // loosely-typed DeprecatedUI keys that the `classNames` prop otherwise accepts,
  // so a renamed or removed key (e.g. after a library upgrade) fails
  // `pnpm run typecheck` instead of silently dropping the custom styling.
  const defaultClassNames: Partial<ClassNames> = {
    root: cn("w-fit", rdpDefaultClassNames.root),
    months: cn(
    ...
    range_start: cn(
      ...
      rdpDefaultClassNames.range_start
    ),
    range_middle: cn("rounded-none", rdpDefaultClassNames.range_middle),
    range_end: cn("bg-accent rounded-r-md", rdpDefaultClassNames.range_end),
    today: cn(
