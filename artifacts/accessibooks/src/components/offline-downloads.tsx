...
// ─── Presentational view ───────────────────────────────────────────────
// Extracted so it can be unit-tested with renderToString without depending
// on IndexedDB, useSubscription, or any browser API.

export interface OfflineDownloadsViewProps {
  isPremium: boolean;
  downloads: Array<{
    bookId: string;
    title: string;
  ...
  activeDownloads: Array<{ bookId: string; progress: number; status: string }>;
  failedDownloads: Array<{ bookId: string; error?: string }>;
  storageUsed: number;
...
export function OfflineDownloadsView({
  ...
  const hasLoanDownloads = downloads.some((d) => !!d.loanId);
  const showUpsell = !isPremium && !hasLoanDownloads;
  ...
  return (
    <div className="space-y-6">
      ...
      {showUpsell && (
        <Card className="border-2 border-dashed border-muted-foreground/30 dark:border-muted-foreground/20">
          <CardContent className="py-6 text-center">
            ...
            <h3 className="text-base font-semibold mb-1">Premium unlocks downloads</h3>
            <p className="text-muted-foreground text-xs max-w-sm mx-auto">
              Premium subscribers can save audiobooks for offline listening.
              You can also download borrowed loans on any plan.
            </p>
          </CardContent>
        </Card>
      )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p
                      ...
                      data-testid={`offline-meta-${book.bookId}`}
                    >
                      {formatBytes(book.sizeBytes)} • Saved{" "}
          ...
          <Card className="border-dashed">
            <CardContent className="py-8 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                Download audiobooks from the library to listen offline.
              </p>
            </CardContent>
          </Card>
        )
      ...
      {/* Always-visible licensing note: lists categories that cannot be
          saved offline regardless of plan or current download state. */}
      <Card className="border-dashed">
        <CardContent className="py-3">
...
export function OfflineDownloads() {
  ...
  const failedDownloads = Object.values(progressMap).filter(
    (p) => p.status === "error",
  );
  ...
  return (
    <OfflineDownloadsView
      ...
      onPlay={(bookId) => {
        const book = downloads.find((d) => d.bookId === bookId);
        if (!book) return;
        // Stamp lastPlayedAt + start playback. Playback is delegated to the
        // global audio player via a custom event the AudioContext consumer
        // can wire into; recording the play timestamp is required by the
        // Storage panel "last played" column.
        void markPlayed(bookId);
        try {
          const url = URL.createObjectURL(book.audioBlob);
          window.dispatchEvent(
            new CustomEvent("offline:play", {
              detail: { bookId, url, title: book.title },
            }),
          );
        } catch {
      ...
      onRemove={(bookId) => removeDownload(bookId)}
    />
  );
}
