...
export async function loadInteractions(): Promise<InteractionDataset> {
  ...
  let rows: Array<{ userId: string; bookId: string; currentTime: number | null; totalDuration: number | null; playCount: number | null; completedAt: Date | null }> = [];
  try {
    rows = await db
      .select({
  ...
  for (const r of rows) {
    ...
    weight = Math.min(1.0, weight + Math.log1p(r.playCount ?? 1) * 0.05);
