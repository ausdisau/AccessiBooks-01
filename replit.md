...
- **Prerendered marketing-page heads (web, production-only)**: `artifacts/accessibooks/scripts/prerender.mjs` runs after `vite build` and writes `dist/public/<route>/index.html` for `/pricing /trust /institutional /accessibility /privacy /terms /copyright` with route-specific title/canonical/description/og/twitter and per-route WebPage/AboutPage JSON-LD (base `index.html` carries the Organization/WebSite @graph). `artifact.toml` production rewrites map each route to its prerendered file BEFORE the `/*` catch-all. Dev Vite always serves the plain `index.html` — verify via build output, not dev curl. **Drift caveat**: the metadata strings are duplicated from each page's `useEffect` (pricing.tsx, trust.tsx, institutional.tsx, legal.tsx) — if you change titles/descriptions there, update `prerender.mjs` too. The regexes fail the build on structural drift but cannot catch string drift.
